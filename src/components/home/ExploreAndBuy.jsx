...
import { exploreAndBuyItems as items } from "../../data/homeData";
    ...
    <div className="px-3 pb-8">
      ...
      >
        ...
        <div className="absolute top-6 left-6 z-20 flex items-center gap-2 bg-slate-950/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
          ...
          <span className="text-white text-[10px] font-bold uppercase tracking-widest">{item.views}k views</span>
        </div>
        ...
        <video
          src={item.video}
          className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-[3000ms]"
          muted
          loop
          playsInline
        />
        ...
        <div className="mt-auto relative z-10 p-8 space-y-6 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 border-white shadow-lg flex-shrink-0 group-hover:scale-110 transition-transform">
              <video src={item.video} className="w-full h-full object-cover" muted loop playsInline />
            </div>
          ...
          <button className="w-full py-4 bg-white text-slate-950 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all hover:bg-primary-gold group-hover:shadow-2xl translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 duration-500">
        ...
        <div className="px-2">
          <Slider {...settings}>
            {items.map((item, index) => (
              <VideoCard key={index} item={item} onClick={setSelectedItem} />
            ))}
          </Slider>
        </div>
      </div>
